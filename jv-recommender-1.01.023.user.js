      ...
      return new Promise((resolve, reject) => {
        GM_xmlhttpRequest({
          ...
          onload: (r) => {
            ...
            if (r.status === 401 || r.status === 403) msg += '（Top250 等个人数据需登录 javdb 账号）';
  ...
  const adapters = {
    javdb: {
      ...
      sources: [
        ...
        { id: 'top250', label: 'Top250（需登录）', mode: 'api', paged: true, pages: 13, apiPath: '/api/v1/movies/top', apiParams: (kw, page) => ({ type: 'all', type_value: 'all', start_rank: '1', page: String(page || 1), limit: '20' }) },
