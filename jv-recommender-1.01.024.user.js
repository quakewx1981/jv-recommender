...
(function () {
  ...
  const adapters = {
    javdb: {
      ...
      sources: [
        ...
        // 匿名高分榜：App API /api/v1/rankings（无需登录），返回全时期高分影片（含老片），替代需登录的 /api/v1/movies/top
        { id: 'rankings_movies', label: '高分榜（全时期，匿名）', mode: 'api', apiPath: '/api/v1/rankings', apiParams: { type: 'all', period: 'monthly' } },
      ...
      async apiFetch(sourceId, keyword) {
        ...
        const base = typeof src.apiParams === 'function' ? src.apiParams(keyword, 1) : Object.assign({}, src.apiParams);
        // 搜索源 / Top250 翻多页（Top250 自定义 pages=13，每页20部，合计约260部）
        ...
        const results = await mapLimit(pageNums, pages, async (pg) => {
          ...
          if (!base.limit) params.limit = CONFIG.pageSize;
          try {
  ...
  function buildPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="jvr-b" id="jvr-body">
        ...
        <label>年代（按发行年，求老片时选；配“高分榜”出“以前的高分”）</label>
        <select id="jvr-year">
  ...
  async function runRecommend(randomize) {
    ...
    if (cat || keyword) {
      ...
      if (sourceId.indexOf('hot_') === 0 || sourceId === 'rankings_movies') sourceId = 'search';
    }
